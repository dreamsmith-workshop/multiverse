...
function(mltvrs_configure_project)
    ...
    # address sanitizer
    if(CMAKE_CXX_COMPILER_ID STREQUAL "Clang" OR CMAKE_CXX_COMPILER_ID STREQUAL "AppleClang")
        if(${PARSED_PREFIX}_ENABLE_ADDRESS_SANITIZER)
            add_compile_options(-fsanitize=address)
            add_link_options(-fsanitize=address)
        endif()
    else()
        message(SEND_ERROR  "option  : ${PARSED_PREFIX}_ENABLE_ADDRESS_SANITIZER")
        message(SEND_ERROR  "compiler: ${CMAKE_CXX_COMPILER_ID}")
        message(FATAL_ERROR "using address sanitizer on this compiler unsupported")
    endif()
    mltvrs_report_option(ENABLE_ADDRESS_SANITIZER)

    # thread sanitizer
    if(CMAKE_CXX_COMPILER_ID STREQUAL "Clang" OR CMAKE_CXX_COMPILER_ID STREQUAL "AppleClang")
        if(${PARSED_PREFIX}_ENABLE_THREAD_SANITIZER)
            add_compile_options(-fsanitize=thread)
            add_link_options(-fsanitize=thread)
        endif()
    else()
        message(SEND_ERROR  "option  : ${PARSED_PREFIX}_ENABLE_THREAD_SANITIZER")
        message(SEND_ERROR  "compiler: ${CMAKE_CXX_COMPILER_ID}")
        message(FATAL_ERROR "using thread sanitizer on this compiler unsupported")
    endif()
    mltvrs_report_option(ENABLE_THREAD_SANITIZER)

    # undefined behavior sanitizer
    if(CMAKE_CXX_COMPILER_ID STREQUAL "Clang" OR CMAKE_CXX_COMPILER_ID STREQUAL "AppleClang")
        if(${PARSED_PREFIX}_ENABLE_UNDEFINED_BEHAVIOR_SANITIZER)
        ...
        message(SEND_ERROR  "option  : ${PARSED_PREFIX}_ENABLE_DETAILED_CONCEPTS_DIAGNOSTICS")
        message(SEND_ERROR  "compiler: ${CMAKE_CXX_COMPILER_ID}")
        message(FATAL_ERROR "using undefined behavior sanitizer on this compiler unsupported")
    endif()
    mltvrs_report_option(ENABLE_UNDEFINED_BEHAVIOR_SANITIZER)
